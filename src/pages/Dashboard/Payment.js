...
const Payment = () => {
  ...
  const { isLoading, data: orders } = useQuery(["order", id], () =>
    fetch(url, {
      method: "GET",
      headers: {
        ...
        <div class="card w-50 max-w-md bg-base-100 shadow-xl my-5">
          <div class="card-body">
            <div className="flex gap-3">
                <div className=" p-1">
                    <h3 class="card-title">Pay for food: <span className="text-neutral font-sans">{orders.name}.</span></h3>
                    <p>Please pay : <span className="text-neutral font-sans">${orders.price}</span></p>
                </div>
                <div className="p-1">
